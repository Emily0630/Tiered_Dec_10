...
sum_outcome = {str(eps_greedy): [], str(boot_ts): [], 'RandomAction': [], 'opt_policy': [], str(ps_eps_greedy):[]}
## Burn-in
...
for t in range(num_steps):
   ...
   # Vanilla Boot TS
    ...
    if t % 1 == 0:
        ...
        print(sum_outcome)
        pd.DataFrame(sum_outcome).to_csv(f'results_temp/comparison_4_{num_steps}.csv', index=False)
