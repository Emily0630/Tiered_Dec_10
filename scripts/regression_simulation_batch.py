...
parser.add_argument('--batch_size', help='Iteration number', type=int)
...
for i in range(B):
    ...
    cmtr_smth = ConstrainedMonotoneTreeRegressor(**cmtr_smth_params)
    ...
    cmtr_smth.fit(X=train_set._surrogate, y = train_set._outcome)
    ...
    y_cmtr_smth = cmtr_smth.predict(X=test_set._surrogate)
    ...
    cmtr_smth_rmse = np.sqrt(mean_squared_error(test_set._outcome, y_cmtr_smth))
    lr_rmse   = np.sqrt(mean_squared_error(test_set._outcome, y_lr))

    results.append((mtr_rmse, cmtr_rmse, cmtr_smth_rmse, lr_rmse))

file_path = os.path.join(dir_path, str(iteration) + '.csv')
print(f'Writing results to {file_path}\n')


pd.DataFrame(results, columns=['MTR', 'CMTR', 'CMTR_SMTH', 'RLR']).to_csv(file_path, index=False)
