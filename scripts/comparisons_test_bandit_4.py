...
sum_outcome = {str(eps_greedy): [], str(boot_ts): [], 'RandomAction': [], 'opt_policy': [], str(ps_eps_greedy):[], 'non-dominated': []}
## Burn-in
...
for t in range(num_steps):
    ...
    sum_outcome["opt_policy"].append(eps_greedy._policies._basket.index(eps_greedy.optimal_policy))
   ...
   # Vanilla Boot TS
    ...
    sum_outcome["non-dominated"].append(ps_eps_greedy._non_dominated_indices)
    # breakpoint()
    ...
    if t % 1 == 0:
        ...
        print(sum_outcome)
        pd.DataFrame(sum_outcome).to_csv(f'results_temp/comparison_5_{num_steps}.csv', index=False)
