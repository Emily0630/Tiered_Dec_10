...
from src.generative_models import CopulaGenerativeModel, TestGenerativeModel, AveGenerativeModel
from src.policy import LinearBasket, LinearBasketBinary
...
num_actions = 2
theta = list()
theta.append(np.ones((p, q))/np.sqrt(p))
theta.append(-np.ones((p, q))/np.sqrt(p))
...
basket = LinearBasketBinary.generate_random_basket(
    num_policies=num_policies,
    x_dim=p
)
...
sum_outcome = {str(eps_greedy): [], str(ps_eps_greedy): [], str(guess): [], "optimal": []}
...
for t in range(num_steps):
    ...
    pd.DataFrame(sum_outcome).to_csv(
        "../results_no_MT/" + f'comparison_0_3_{t}_{num_steps}.csv',
        index=False
    )
    print(
