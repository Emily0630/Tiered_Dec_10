...
from src.generative_models import CopulaGenerativeModel, TestGenerativeModel
...
# Parameters for monotonic tree embedding
n_trees = 100
n_estimators = 5
...
for t in range(num_steps):
    ...
    t1 = time.time()
    # Vanilla Epsilon Greedy
    x = gen_model.get_context(n)  # Should only generate context once
    ...
    # Policy Screening  Epsilon Greedy
    #x = gen_model.get_context(n)  # use same context across all algorithms
    print("Checkpoint: about to pick action")
    ...
    bandit_opt = bandit_opt.add_observations(
        context_new=x,
        action_new=a,
        surrogate_new=z,
        outcome_new=y
    ...
    pd.DataFrame(sum_outcome).to_csv(
        "../results_no_MT/" + prepend + f'_comparison_0_1_{t}_{num_steps}.csv',
        index=False
    )
    print(
