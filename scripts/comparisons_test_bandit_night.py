...
sum_outcome = {str(eps_greedy): [], str(boot_ts): [], 'RandomAction': [], 'opt_policy': []}
## Burn-in
...
for t in range(num_steps):
   ...
   # Vanilla Boot TS
    ...
    if t % 1 == 0:
        ...
        print(sum_outcome)
        pd.DataFrame(sum_outcome).to_csv(f'results_temp/comparison_night_{num_steps}.csv', index=False)
        # breakpoint()
        print(f"Optimal policy: {eps_greedy._policies._basket.index(eps_greedy.optimal_policy)}")
